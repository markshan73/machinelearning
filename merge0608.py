# -*- coding: utf-8 -*-
"""
Created on Tue Jun  8 13:13:54 2021
...
at_num=replace_outliers_with_nan(at_num,3)#帶入application_train.csv 中數值欄位、以3個標準差計算離群值(307511,82)
atest_num=replace_outliers_with_nan(atest_num,3)#帶入application_test.csv 中數值欄位、以3個標準差計算離群值(48744,82)

############################################################################################
#                      數值資料標準化[新資料=（原始資料-均值）/標準差]
############################################################################################
#at_num.set_index('SK_ID_CURR', inplace=True)#將SK_ID_CURR設為index(307511,81)
#at_num=(at_num-at_num.mean())/(at_num.std())#新資料=（原始資料-均值）/標準差(307511,81)
#at_num.reset_index(inplace=True)#回復原來的index

#atest_num.set_index('SK_ID_CURR', inplace=True)#將SK_ID_CURR設為index(48744,81)
#atest_num=(atest_num-atest_num.mean())/(atest_num.std())#新資料=（原始資料-均值）/標準差(48744,81)
#atest_num.reset_index(inplace=True)#回復原來的index(48744,82)
############################################################################################
#                      類別欄位 
############################################################################################
...
#訓練集 保留與測試集欄位的交集
at_merge=at_merge[unicl]#(307511,354)
at_targe=at.loc[:,['SK_ID_CURR','TARGET']]#訓練集 的'SK_ID_CURR','TARGET'欄位
at_merge=at_merge.merge(at_targe, on='SK_ID_CURR',how='left')#(307511,355)#將 'TARGET'欄位加入訓練集
train_labels = at_merge['TARGET']
train=at_merge.drop(['TARGET'], axis=1)
at_merge.to_csv('at_merge.csv')#匯出.csv
#測試集 保留與訓練集欄位的交集
atest_merge=atest_merge[unicl]##(48744,354)
test=atest_merge
atest_merge.to_csv('atest_merge.csv')#匯出.csv
############################################################################################
#  採用的方法與方法的簡介以及使用此方法的想法
############################################################################################
############################################################################################
#  實證的結果以及績效的評估
#############################################################################################
...
grid_search.fit(train, train_labels,returntrain)
